...
public class ExtractTable {
	...
	public void  testPrintPROVStatements(){
		...
		Model activities = statements.filter(null, RDF.TYPE, PROVReporter.provActivityURI);
		...
		for(Resource sActivity : sbjActivities){
			...
			Model curInputs  = statements.filter(sActivity, PROVReporter.provUsedURI, null);
			
			// Outputs which are generated by current activity
			Model curOutputs = statements.filter(null, PROVReporter.provWasGeneratedByURI, sActivity);
			...
			for(Value v : curInputs.objects()){
				Model val = statements.filter((Resource)v, PROVReporter.d2sValueURI, null);
				System.out.print("\t "+v+ " : " );
			...
			for(Resource r : curOutputs.subjects()){
				Model val = statements.filter(r, PROVReporter.d2sValueURI, null);
